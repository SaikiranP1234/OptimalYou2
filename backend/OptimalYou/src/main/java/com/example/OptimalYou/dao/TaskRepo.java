...
public interface TaskRepo extends JpaRepository<Task, Integer> {
    ...
    @Query("FROM Task t WHERE t.user.username = :username AND t.deadline >= :date AND t.completed = false")
    ...
    @Query("FROM Task t WHERE t.user.username = :username AND t.completed = true")
    List<Task> findByUsernameAndCompleted(String username);

    @Query("FROM Task t WHERE t.user.username = :username AND (t.taskTitle LIKE %:searchTerm% OR t.taskDescription LIKE %:searchTerm%)")
