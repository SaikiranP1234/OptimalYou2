document.addEventListener("DOMContentLoaded", async () => {
    ...
    container.innerHTML = "<p>Generating your personalized advice...</p>";

    const fullData = JSON.parse(localStorage.getItem("finance-data"));
    if (!fullData) {
    ...
    try {
        const response = await fetch("http://localhost:8080/ai/generateAdvice", {
            method: "POST",
